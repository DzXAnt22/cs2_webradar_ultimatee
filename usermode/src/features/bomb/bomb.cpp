...
void f::bomb::get_carried_bomb(c_base_entity* bomb)
{
    ...
    m_data["m_bomb"] = {
        {"x", scene_origin.m_x},
        {"y", scene_origin.m_y},
        {"m_x", scene_origin.m_x},
        {"m_y", scene_origin.m_y},
        {"position", {{"x", scene_origin.m_x}, {"y", scene_origin.m_y}}},
...
void f::bomb::get_planted_bomb(c_planted_c4* planted_c4)
{
    ...
    auto vec_origin = scene_node->m_vecAbsOrigin();
    if (vec_origin.is_zero())
        vec_origin = scene_node->m_vecOrigin();
    ...
    const auto blow_time = std::max(planted_c4->m_flC4Blow() - curtime, 0.f);
    ...
    const auto raw_defuse_time = planted_c4->m_flDefuseCountDown() - curtime;
    const auto defuse_time = is_defusing ? std::max(raw_defuse_time, 0.f) : 0.f;
    ...
    m_data["m_bomb"] = {
        ...
        {"m_x", vec_origin.m_x},
        {"m_y", vec_origin.m_y},
        {"position", {{"x", vec_origin.m_x}, {"y", vec_origin.m_y}}},
